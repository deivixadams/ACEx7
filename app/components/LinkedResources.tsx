...
const LinkedResources: React.FC<LinkedResourcesProps> = ({ controls, tests, isLoading }) => {
    if (isLoading) {
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full">
                <div className="px-5 py-3 border-b border-slate-100 bg-amber-50/50 flex items-center justify-between">
                    <h3 className="font-bold text-slate-700 uppercase tracking-wide text-xs flex items-center gap-2">
                        ...
                        Controles Mitigantes ({controls.length})
                    </h3>
                </div>
                        ...
                        <table className="w-full text-left text-xs">
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {controls.map((control) => (
                                    <tr key={control.id} className="hover:bg-slate-50/50">
                                        <td className="px-5 py-2.5 font-mono text-slate-500">{control.codigo || 'N/A'}</td>
                                        <td className="px-5 py-2.5 font-medium text-slate-700">{control.nombre}</td>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full">
                <div className="px-5 py-3 border-b border-slate-100 bg-blue-50/50 flex items-center justify-between">
                    <h3 className="font-bold text-slate-700 uppercase tracking-wide text-xs flex items-center gap-2">
                        ...
                        Pruebas de Auditoría ({tests.length})
                    </h3>
                </div>
                ...
                <div className="flex-1 overflow-auto max-h-[400px]">
                    {tests.length === 0 ? (
                        ...
                        <table className="w-full text-left text-xs">
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {tests.map((test) => (
                                    <tr key={test.id_prueba} className="hover:bg-slate-50/50">
                                        <td className="px-5 py-2.5 font-mono text-slate-500">{test.codigo || 'N/A'}</td>
                                        <td className="px-5 py-2.5 font-medium text-slate-700">{test.nombre}</td>
